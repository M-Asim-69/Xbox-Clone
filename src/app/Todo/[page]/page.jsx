  ...
  return (
    <div className="relative bg-gradient-to-b from-gray-500 to-black text-white min-h-screen p-8">
      ...
      <div className="relative z-10">
        <h1 className="text-4xl mt-24 font-bold text-green-500 mb-6 text-center">Your Todos</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {todos.map((todo) => (
            ...
            >
              <h2 className="text-2xl font-semibold text-green-400 mb-3">{todo.title}</h2>
              <p className="text-gray-300 mb-4">{todo.description}</p>
              <div className="mt-4 flex justify-between">
                <Delete id={todo.id} />
                <div className="ml-4">
                  <Link
                    href={`/Update/${todo.id}?title=${encodeURIComponent(todo.title)}&description=${encodeURIComponent(todo.description)}`}
                  >
                    <button className="bg-yellow-600 py-2 shadow-lg rounded-lg px-4">Update</button>
                  </Link>
                </div>
              </div>
            </div>
